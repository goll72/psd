...
Enquanto o sinal de \textit{reset} do circuito estiver acionado, estamos no primeiro estado da \textit{FSM}, \textit{RESET}.
Em seguida, o estado \textit{FETCH} é atingido, neste momento é buscado o conteúdo no endereço de memória que se
encontra no \textit{PC} (\textit{Progam Counter}). Depois temos o estado \textit{STORE}, em que o valor encontrado
é guardado nos registradores \textit{IR} (\textit{Instruction Register}) e \textit{RS} (\textit{Register Select}),
caso na instrução esteja indicado que é necessário o imediato, buscamos ele na memória durante o estado
(\textit{FETCH IMM}) e depois é colocado no registrador durante o estado \textit{STORE IMM}. 
Por fim executamos tal instrução. Caso a instrução não seja uma operação \textit{WAIT}, volta-se para o estado
\textit{FETCH} novamente. senão entra-se no estado \textit{POLL}, na qual é aguardado até o sinal \textit{INT}
ser acionado, voltando para o estado \textit{FETCH}.
